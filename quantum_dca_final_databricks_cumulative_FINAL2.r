...
eur_weight<- function(q, forecast) {
  ...
  print(paste(eur_diff, eur12_diff))
...
hyp2exp.transition <- function (Di, b, Df) {
  # print(paste(Di, b, Df))
  if (is.na(Di) || is.na(b) || is.na(Df)) {
    # Handle the case where any of Di, b, or Df is NA
    warning("One or more of Di, b, or Df is NA. Returning NA.")
    return(0)
  }
  ...
  function (q,
    ...
    if (length(q) != length(t)) {
      ...
      if (length(q) >= 6) {
        ...
        print(paste('di guess is coming here'))
        di_guess <-
          ifelse(
            calc_decl < 0,
            min(
      ...
      print(paste("initial guess", initial.guess))
      goal.function.huber <- function (guess, texp) {
      ...
      goal.function.ssle <- function (guess, texp) {
        q.forecast <- multi_segment2.q(
          qi = guess[1],
          Di = guess[2],
          b =  guess[3],
      ...
      scaling_hl <- c(1000, 10, 5, 1, 10)
      scaling_ssle <- c(1000, 4.176091, 5.045323, 1, 4.176091)

      texp_idx_arr <- seq(1, 12, 1)
      r_list_hubber <- list()

      # start_time <- Sys.time()

      # for (i in texp_idx_arr){
      #   res.huber <- try(optim(
      #     par = initial.guess,
      ...
      #   ),
      #   silent = TRUE)
      #   r_list_hubber[[length(r_list_hubber) + 1]] <- res.huber
      # }
      #  end_time <- Sys.time()

      # Calculate elapsed time
      # elapsed_time <- end_time - start_time
      # print(paste("time taken 1", elapsed_time))


      # start_time <- Sys.time()

      r_list_hubber <- lapply(texp_idx_arr, function(i) {
        res.huber <- try(optim(
          par = initial.guess,
          fn = goal.function.huber,
          texp = i,
      ...
      # # Calculate elapsed time
      # elapsed_time <- end_time - start_time
      # print(paste("time taken 2", elapsed_time))

      fun_Val_huber <- numeric()
      for (result in r_list_hubber) {
        fun_Val_huber <- c(fun_Val_huber, result[[2]])
      }
      opt_idx_huber <- which.min(fun_Val_huber)
      ...
      #   r_list_ssle[[length(r_list_ssle) + 1]] <- res.ssle
      # }

      r_list_ssle <- lapply(texp_idx_arr, function(i) {
        res.ssle <- try(
          optim(
            par = initial.guess,
            fn = goal.function.ssle,
      ...
      fun_Val_ssle <- numeric()
      for (result in r_list_ssle) {
        fun_Val_ssle <- c(fun_Val_ssle, result[[2]])
      }

      opt_idx_ssle <- which.min(fun_Val_ssle)
      ...
      huber.predict <-
        multi_segment2.q(
          qi = r_list_hubber[[opt_idx_huber]]$par[1],
          Di = r_list_hubber[[opt_idx_huber]]$par[2],
          b =  r_list_hubber[[opt_idx_huber]]$par[3],
          Df = r_list_hubber[[opt_idx_huber]]$par[4],
          alpha = r_list_hubber[[opt_idx_huber]]$par[5],
          opt_idx_huber,
          t
      )

      # eur12_diff <- ((sum(q[1:12]) - sum(huber.predict[1:12])) * 100) / sum(q[1:12])
      # eur_diff <- ((sum(q) - sum(huber.predict)) * 100) / sum(q)

      # huber.loss.q <-
      #   sum(abs(q[loss_lim:loss_lim2] - huber.predict[loss_lim:loss_lim2]), na.rm = TRUE)
      huber.loss.q <- eur_weight(q, huber.predict)

      print(paste(r_list_ssle[[opt_idx_ssle]]$par))

      ssle.predict <-
        multi_segment2.q(
          qi = r_list_ssle[[opt_idx_ssle]]$par[1],
          Di = r_list_ssle[[opt_idx_ssle]]$par[2],
          b =  r_list_ssle[[opt_idx_ssle]]$par[3],
          Df = r_list_ssle[[opt_idx_ssle]]$par[4],
          alpha = r_list_ssle[[opt_idx_ssle]]$par[5],
          opt_idx_ssle,
          t
        )
      # eur12_diff <- ((sum(q[1:12]) - sum(ssle.predict[1:12])) * 100) / sum(q[1:12])
      # eur_diff <- ((sum(q) - sum(ssle.predict)) * 100) / sum(q)

      # ssle.loss.q <-
      #   sum(abs(q[loss_lim:loss_lim2] - ssle.predict[loss_lim:loss_lim2]), na.rm = TRUE)
      ssle.loss.q <- eur_weight(q, ssle.predict)
      print(paste("huber loss", huber.loss.q))
      print(paste("ssle loss", ssle.loss.q))
      print(ssle.predict[1:10])
      print(q[1:10])
      if (abs(huber.loss.q) < abs(ssle.loss.q)) {
        res <- r_list_hubber[[opt_idx_huber]]
        scaling <- scaling_hl
        goal_fn <- goal.function.huber
        texp_idx <- opt_idx_huber
      } else {
        res <- r_list_ssle[[opt_idx_ssle]]
        scaling <- scaling_ssle
        goal_fn <- goal.function.ssle
        texp_idx <- opt_idx_ssle
      }
